...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName("create_or_delete_or_login").setDescription("backupアカウントの削除または作成またはログインです。").addChoices({ name: 'create', value: 'create' }, { name: 'delete', value: 'delete' }, { name: 'login', value: 'login' }).setRequired(true))
    ...
    async execute(interaction) {
        ...
        if (interaction.options.getString("create_or_delete_or_login") === "create") {
            const data = await func.dbget({ table: "account", v: `id="${interaction.user.id}"` });
            ...
            const check = await func.dbget({ table: "account", v: `name="${interaction.options.getString("name")}"` });
            ...
            const set = await func.dbset({ table: "account", v: `"${interaction.options.getString("name")}","${interaction.options.getString("pass")}","${interaction.user.id}"` });
        ...
        if (interaction.options.getString("create_or_delete_or_login") === "delete") {
            ...
            await func.dbdelete({ table: "account", v: `name="${interaction.options.getString("name")}" and pass="${interaction.options.getString("pass")}"` });
            await interaction.reply({ embeds: [{ title: "お知らせ", description: `完了しました。`, color: 0x3aeb34 }], ephemeral: true })
        };
        if (interaction.options.getString("create_or_delete_or_login") === "login") {
            ...
            if (check[0].pass !== interaction.options.getString("pass")) return await interaction.reply({ embeds: [{ title: "エラー", description: `パスワードが間違っています。`, color: 0x3aeb34 }], ephemeral: true });
            const update = await func.dbupdate({ table: "account", set: `id="${interaction.user.id}"`, v: `name="${interaction.options.getString("name")}" and pass="${interaction.options.getString("pass")}"` });
