...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('shop_delete')
        .addStringOption(option => option.setName("item_or_account").setDescription("商品の削除またはshopアカウントの削除です。").addChoices({ name: 'item', value: 'item' }, { name: 'account', value: 'account' }).setRequired(true))
    ...
    async execute(interaction) {
        if (interaction.options.getString("item_or_account") === "account") {
            ...
            await func.dbdelete({ table: "paypay", v: `id="${interaction.user.id}"` });
            const goods = await func.dbget({ table: "goods", v: `username="${paypay[0].name}"` });
            if (!goods[0]?.id) return await interaction.reply({ embeds: [{ title: "お知らせ", description: `完了。`, color: 0x3aeb34 }], ephemeral: true });
            await func.dbdelete({ table: "goods", v: `username="${paypay[0].name}"` });
            const item = await func.dbget({ table: "inventory", v: `id="${goods[0].id}"` });
            if (!item[0]?.id) return await interaction.reply({ embeds: [{ title: "お知らせ", description: `完了。`, color: 0x3aeb34 }], ephemeral: true });
            await Promise.all(goods.map(async v => await func.dbdelete({ table: "inventory", v: `id="${v.id}"` })));
        ...
        if (interaction.options.getString("item_or_account") === "item") {
            ...
            const info = await func.dbget({ table: "goods", v: `name="${name}" and username="${paypay[0].name}"` });
            if (!info[0]?.name) return await interaction.reply({ embeds: [{ title: "エラー", description: `商品が見つかりません。`, color: 0x3aeb34 }], ephemeral: true });
            await func.dbdelete({ table: "goods", v: `name="${name}" and username="${paypay[0].name}"` });
            await Promise.all(info.map(async v => await func.dbdelete({ table: "inventory", v: `id="${v.id}"` })));
            await interaction.reply({ embeds: [{ title: "お知らせ", description: `完了。`, color: 0x3aeb34 }], ephemeral: true });
        };
    }
};