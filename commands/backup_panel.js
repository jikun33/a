...
module.exports = {
    ...
    async execute(interaction) {
        ...
        await interaction.guild.members.me.roles.add(option.getRole("role")).then(async () => {
            const name = await func.dbget({ table: "account", v: `id="${interaction.user.id}"` });
            ...
            const backup = await func.dbget({ table: "backup", v: `guildid="${interaction.guild.id}"` });
            if (backup[0]?.username) {
                const update = await func.dbupdate({ table: "backup", set: `username="${name[0].name}"`, v: `guildid="${interaction.guild.id}"` });
                if (!update) return await interaction.reply({ embeds: [{ title: "エラー", description: "データの更新に失敗しました。", color: 0x3aeb34 }], ephemeral: true });
            } else {
                const set = await func.dbset({ table: "backup", v: `"${name[0].name}","${interaction.guild.id}"` });
                if (!set) return await interaction.reply({ embeds: [{ title: "エラー", description: "データの保存に失敗しました。", color: 0x3aeb34 }], ephemeral: true });
            };
            const check = await func.dbget({ table: "verify", v: `guildid="${interaction.guildId}"` });
            if (check[0]?.roleid) {
                const update = await func.dbupdate({ table: `verify`, set: `roleid="${option.getRole("role").id}"`, v: `guildid="${interaction.guildId}"` });
                if (!update) return await interaction.reply({ embeds: [{ title: "エラー", description: "データの更新に失敗しました。", color: 0x3aeb34 }], ephemeral: true });
            } else {
                const set = await func.dbset({ table: "verify", v: `${interaction.guildId},${option.getRole("role").id}` });
                if (!set) return await interaction.reply({ embeds: [{ title: "エラー", description: "データの保存に失敗しました。", color: 0x3aeb34 }], ephemeral: true });
            };
