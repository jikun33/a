...
module.exports = {
    ...
    async execute(interaction) {
        const get_paypay = await func.dbget({ table: "paypay", v: `id="${interaction.user.id}"` });
        if (!get_paypay[0]?.id) return await interaction.reply({ embeds: [{ title: "エラー", description: `アカウントがありません。`, color: 0x3aeb34 }], ephemeral: true });
        const blink = interaction.options.getString("paypaylink");
        if (setting.discord.admin == interaction.user.id) {
            const now = func.nowdate();
            const date = format(func.getAddMonthDate(now[0], now[1], now[2], setting.month), "yyyy-MM-dd", { timeZone: "Asia/Tokyo" });
            const update = await func.dbset({ table: `subsc`, v: `"${get_paypay[0].name}","${date}"` });
            if (!update) return await interaction.reply({ embeds: [{ title: "エラー", description: `データの保存に失敗しました。`, color: 0x3aeb34 }], ephemeral: true });
            await interaction.reply({ embeds: [{ title: "おしらせ", description: `${setting.month}ヶ月分有効になりました。`, color: 0x3aeb34 }], ephemeral: true });
        } else {
            ...
            if (!response?.data?.header) await interaction.reply({ embeds: [{ title: "エラー", description: `ホストのPayPayアカウントが無効です。`, color: 0x3aeb34 }], ephemeral: true });
            ...
            if (response.data.header.resultCode == "S0000") {
                ...
                if (response.data.payload.orderStatus == "PENDING") {
                    ...
                    if (r.data.header.resultCode == "S0000") {
                        const usercheck = await func.dbget({ table: "subsc", v: `name="${get_paypay[0].name}"` });
                        if (usercheck[0]?.name) {
                            const now = func.nowdate(usercheck[0]?.subsc);
                            const date = format(func.getAddMonthDate(now[0], now[1], now[2], setting.month), "yyyy-MM-dd", { timeZone: "Asia/Tokyo" });
                            const update = await func.dbupdate({ table: `subsc`, set: `subsc="${date}"`, v: `name="${get_paypay[0].name}"` });
