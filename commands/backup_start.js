...
module.exports = {
    ...
    async execute(interaction) {
        const adata = await func.dbget({ table: "account", v: `id="${interaction.user.id}"` });
        ...
        const backupdata = await func.dbget({ table: "backup", v: `username="${adata[0].name}"` });
        ...
        const backupdata2 = await func.dbget({ table: "backup", v: `guildid="${interaction.options.getString("guildid")}"` });
        ...
        const users = await func.dbget({ table: "guild", v: `guildid="${interaction.options.getString("guildid")}"` });
        ...
        await Promise.all(datas.map(async id => {
            ...
            await sleep(4000);
            const tokens = await func.dbget({ table: `token`, v: `userid="${id}"` });
            ...
            const data = await axios.post("https://discord.com/api/oauth2/token", {
                ...
                await func.dbdelete({ table: `token`, v: `userid="${get_data.userid}"` });
            });
            if (!data?.data) return await interaction.editReply({ embeds: [{ title: "お知らせ", description: `リクエストに失敗`, color: 0x3aeb34 }], ephemeral: true });;
            const update = await func.dbupdate({ table: `token`, set: `token="${data.data.refresh_token}"`, v: `userid="${get_data.userid}"` });
